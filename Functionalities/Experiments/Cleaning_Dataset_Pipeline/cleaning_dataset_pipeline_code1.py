...
# Find the folder for the packages dynamically
package_root_dir = find_folder(current_dir, packages_folder)

# If the folder where the package is stored is found, append it to sys.path
if package_root_dir:
    sys.path.append(package_root_dir)  # Add the correct folder to sys.path
    print(f"Package Path: {package_root_dir}")
else:
    print(f"Error: {package_root_dir} folder not found.")
    raise FileNotFoundError(f"{package_root_dir} folder could not be found. Exiting.")

# Now import the 'clean_dataset_pipeline' after adding the package path to sys.path
try:
    from Cleaning_Dataset_Pipeline import clean_dataset_pipeline
except ImportError as e:
    print(f"Error importing 'Cleaning_Dataset_Pipeline': {e}")
    raise ImportError("Failed to import the 'Cleaning_Dataset_Pipeline' module.")
...
if data_folder_dir:
    ...
    raise FileNotFoundError(f"{data_folder_dir} folder could not be found. Exiting.")
...
if not os.path.isfile(input_file):
    ...
    raise FileNotFoundError(f"Input file '{input_file}' does not exist. Exiting.")
...
# Find the 'Keys' folder dynamically
keys_folder_dir = find_folder(current_dir, keys_folder)

# If 'Keys' folder is found, proceed
if keys_folder_dir:
    print(f"Keys folder found: {keys_folder_dir}")
else:
    print(f"Error: {keys_folder_dir} folder not found.")
    raise FileNotFoundError(f"{keys_folder_dir} folder could not be found. Exiting.")

# Define a function to find specific folders inside the folder for keys
...
# Call the cleaning function with the input and output file paths
try:
    clean_dataset_pipeline(input_file, output_file, obfuscations_file)
