...
output_subfolder = os.path.join(data_folder, f'lda_topics_data_2000epochs_{timestamp}')
...
model_file = os.path.join(data_folder, 'lda_model1.pkl')
if not os.path.isfile(model_file):
    model_file = os.path.join(output_subfolder, f'lda_model_{timestamp}.pkl')
else:
...
try:
    from LDA_Topic_Modeling import topic_modeling_initializer
except ImportError as e:
    raise ImportError(f"Failed to import 'LDA_Topic_Modeling': {e}")
...
output_file = os.path.join(output_subfolder, f'lda_topics_2000epochs_{timestamp}.txt')
print(f"Output File: {output_file}")

# Create a file to save metrics
metrics_file = os.path.join(output_subfolder, f'lda_metrics_2000epochs_{timestamp}.txt')
