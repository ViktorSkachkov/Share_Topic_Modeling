...
class TopicModelingPipelineVocabulary:
    def __init__(self, file_path: str, model_file: str, output_file: str,
               ...
               num_top_words: int, epochs: int, vocabulary_file: str):
    ...
    def load_vocabulary(self, vocabulary_file: str) -> set:
        """
        Loads vocabulary words from a text file.
    
        Args:
          vocabulary_file: Path to the file containing vocabulary words.
        ...
        if os.path.isfile(vocabulary_file):
          with open(vocabulary_file, 'r', encoding='utf-8') as f:
            vocabulary.update(f.read().splitlines())
        else:
          print(f"Warning: Vocabulary file not found: {vocabulary_file}")
        return vocabulary
    ...
    def calculate_umass_coherence(self, filtered_dataset):
        ...
        texts = [text.split() for text in filtered_dataset]  # Split the processed text back into tokens
    
        # Create a Gensim Dictionary from the texts
        self.dictionary = Dictionary(texts)
    
        # Use the FASTopic model to retrieve topics
        # topics = [self.model.get_topic(topic_idx=i, num_top_words=self.num_top_words) for i in range(self.num_topics)]
    ...
    def run(self):
        ...
        if os.path.exists(self.model_file):
            ...
            for text in dataset:
                ...
                processed_dataset.append(' '.join(processed_text))  # Join tokens back into a single string
            
            self.filtered_dataset = [text2 for text2 in processed_dataset if any(word in text2 for word in self.vocabulary)]
            self.apply_fastopic(self.filtered_dataset)
            self.save_model()
        ...
        self.calculate_umass_coherence(self.filtered_dataset)
